...
router.post('/push', async (req: Request, res: Response) => {
    try {
        ...
        try {
            // 1. Switch to personal identity so Vercel recognises the contributor
            ...
            // 2. Amend so the commit author reflects personal identity (angheloamir)
            await runGit(['commit', '--amend', '--no-edit', '--reset-author']);

            // 3. Push to dev — Vercel will see angheloamir as the author on this push
            const currentBranch = await runGit(['rev-parse', '--abbrev-ref', 'HEAD']);
            await runGit(['push', 'dev', currentBranch, '--force']);

            // 4. Switch back to work identity BEFORE rewriting local history
            await runGit(['config', 'user.name', 'Clean and Seal']);
            await runGit(['config', 'user.email', 'cleanandsealdp@gmail.com']);

            // 5. Amend the LOCAL commit author back to work identity.
            //    This rewrites only the local commit — we do NOT push again so the
            //    remote (which Vercel already read) stays untouched.
            //    Net result: remote shows angheloamir, local history shows only work account.
            await runGit(['commit', '--amend', '--no-edit', '--reset-author']);

            res.json({ success: true });
        } finally {
            // 6. Always guarantee work identity is restored, even on failure
            try {
                await runGit(['config', 'user.name', 'Clean and Seal']);
                await runGit(['config', 'user.email', 'cleanandsealdp@gmail.com']);
            } catch (configError) {
